--[[
    AUTOWALK v11.3 - NO FREEZE
    - Fix freeze saat playback
    - Safety check untuk deltaTime
    - Pcall di dalam heartbeat
    - Speed karakter kembali normal setelah stop
]]
...
local VERSION = "11.3"
local DATA_FOLDER = "AutoWalkRecords"
...
local originalWalkSpeed = 16  -- untuk simpan kecepatan asli karakter
...
local function startRecording()
    ...
    print("[AutoWalk] Recording started")
...
local function stopRecording()
    ...
    print("[AutoWalk] Recording stopped. Total frames:", #recordedFrames)
    ...
    -- Trim awal untuk menghilangkan diam di awal
    local startIdx = 1
    ...
    print("[AutoWalk] Ready to save, frames:", #pendingFrames)
end
...
local function refreshRecords()
    ...
    print("[AutoWalk] Records refreshed, count:", #savedRecords)
end

-- ==================== MERGE (tanpa jeda & tanpa diam) ====================
local function mergeAllCheckpoints()
    ...
    for cpIdx, cpData in ipairs(cpDataList) do
        ...
        -- Cari frame pertama dan terakhir yang bergerak
        local startIdx = 1
        ...
        -- Jika tidak ada gerakan sama sekali, gunakan semua frame
        if startIdx > endIdx then
            startIdx = 1
            endIdx = nFrames
        end

        -- Ambil hanya bagian yang bergerak
        local movingFrames = {}
        for i = startIdx, endIdx do
            table.insert(movingFrames, frames[i])
        end

        -- Jika ini checkpoint pertama, waktu dimulai dari 0
        if cpIdx == 1 then
            ...
            -- Untuk checkpoint selanjutnya, sambungkan waktu secara berurutan
            local firstTime = movingFrames[1].time
            for _, f in ipairs(movingFrames) do
                local newFrame = {}
                for k,v in pairs(f) do newFrame[k] = v end
...
local function startWalking()
    ...
    -- Simpan kecepatan asli karakter
    originalWalkSpeed = hum.WalkSpeed

    -- Hitung offset Y agar tidak melayang
    ...
    walkHB = RunService.Heartbeat:Connect(function()
        ...
        if not success then
            print("[AutoWalk] Playback error:", err)
        end
    end)
end
...
local function createGUI()
    local old = LP.PlayerGui:FindFirstChild("AutoWalkV9")
    ...
    titleLbl.Text = "🚶 AUTOWALK v"..VERSION
    ...
    stpBtn.MouseButton1Click:Connect(function()
        ...
        task.defer(function()
            ...
            if saved then
                refreshRecords()
                renderList()
                print("[AutoWalk] Record saved and list refreshed:", saved)
            else
                print("[AutoWalk] Save failed")
            end
        end)
    end)
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == Enum.KeyCode.F then
        local g = LP.PlayerGui:FindFirstChild("AutoWalkV9")
        if g then
...
print("====================================")
print("🚶 AUTOWALK v"..VERSION.." — NO FREEZE")
print("✅ Fix freeze saat playback")
print("✅ Safety check untuk deltaTime")
print("✅ Pcall di dalam heartbeat")
print("✅ Speed kembali normal setelah stop")
print("====================================")